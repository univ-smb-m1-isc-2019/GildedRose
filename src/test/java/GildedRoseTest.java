...
public class GildedRoseTest {
	...
	public void testUpperLimitQualityNeverHigher50(){
		...
		for (int i = 0; i < 100; i++){ inn.updateQuality(); }
	...
	public void testLowerLimitQualityNeverUnder0(){
		GildedRose inn = new GildedRose();
		for (int i = 0; i < 10; i++){ inn.updateQuality(); }
		inn.getGoods().getItems().stream()
				.forEach(x -> {
	...
	///////////////////////////////////////
	@Test
	public void testBackstageHighSellIn(){
		GildedRose inn = new GildedRose();
		...
		while (backstage.sellIn > 10){
			...
			assertThat(backstage.quality).isEqualTo(initialQuality + 1);
	...
	@Test
	public void testBackstageLowSellIn(){
		GildedRose inn = new GildedRose();
		Item backstage = inn.get("Backstage passes to a TAFKAL80ETC concert");
		int initialQuality = backstage.quality;
		for(int i = 0; i < 100; i++){
			...
			if (backstage.sellIn < 5 && backstage.sellIn > 0){
				assertThat(backstage.quality).isEqualTo(initialQuality + 3);
			}
			initialQuality = backstage.quality;
		}
	}
	...
	@Test
	public void testBackstageNoSellIn(){
		GildedRose inn = new GildedRose();
		Item backstage = inn.get("Backstage passes to a TAFKAL80ETC concert");
		for(int i = 0; i < 100; ++i){
			inn.updateQuality(backstage);
	...
	@Test
	public void testManaCakeMinusFourQuality(){
		GildedRose inn = new GildedRose();
		Item cake = inn.get("Conjured Mana Cake");
		int initialQuality = cake.quality;
		for(int i = 0; i < 100; i++){
			inn.updateQuality(cake);
			if (cake.sellIn < 0 && cake.quality < 50 && cake.quality > 50){
				assertThat(cake.quality).isEqualTo(initialQuality - 4);
			}
			initialQuality = cake.quality;
		}
	}
	...
	@Test
	public void testCommonItemMinusOneQuality(){
		...
		for (int i = 0; i < 30; i++) {
			...
			if (vest.sellIn >= 0) { assertThat(vest.quality).isEqualTo(initialQualityVest - 1 ); }
			inn.updateQuality(elixir);
			if (elixir.sellIn >= 0)	{ assertThat(elixir.quality).isEqualTo(initialQualityElixir - 1 ); }
	...
	public void testCommonItemMinusTwoQuality(){
		...
		for (int i = 0; i < 30; i++) {
			inn.updateQuality(vest);
			if (vest.sellIn < 0 /*&& vest.quality < 50*/ && vest.quality > 0) { assertThat(vest.quality).isEqualTo(initialQualityVest - 2 ); }
			inn.updateQuality(elixir);
			if (elixir.sellIn < 0 && elixir.quality < 50 && elixir.quality > 0)	{ assertThat(elixir.quality).isEqualTo(initialQualityElixir - 2 ); }
			initialQualityVest = vest.quality;
			initialQualityElixir = elixir.quality;
